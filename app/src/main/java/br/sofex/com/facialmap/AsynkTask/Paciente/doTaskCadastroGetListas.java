...
public class doTaskCadastroGetListas  extends AsyncTask<String, Void, List<String>> {
    ...
    public doTaskCadastroGetListas(Context context,Integer Option){
        mContext = context;
        this.Option = Option;
    }
    ...
    protected List<String> doInBackground(String... params) {
        ...
        List<String> ListResult = new ArrayList<>();
         switch(Option){
             case 1:
             {
                 ListResult = FacialMapRoomDatabase.getDatabaseMapa(mContext).pacienteDao().GetNomePacienteList();
                 break;
             }
             case 2:
             {
                 ListResult = FacialMapRoomDatabase.getDatabaseMapa(mContext).pacienteDao().GetDocumentoList();
                 break;
             }
             case 3:
             {
                 ListResult = FacialMapRoomDatabase.getDatabaseMapa(mContext).pacienteDao().GetDocumentoConteudo();
                 break;
             }
             case 4:
             {
                 ListResult = FacialMapRoomDatabase.getDatabaseMapa(mContext).pacienteDao().GetNacionalidadeList();
                 break;
             }
             case 5:
             {
                 ListResult = FacialMapRoomDatabase.getDatabaseMapa(mContext).pacienteDao().GetDataNascList();
                 break;
             }
             case 6:
             {
                 ListResult = FacialMapRoomDatabase.getDatabaseMapa(mContext).pacienteDao().GetTelFixoList();
                 break;
             }

             case 7:
             {
                 ListResult = FacialMapRoomDatabase.getDatabaseMapa(mContext).pacienteDao().GetTelCelularList();
                 break;
             }
             case 8:
             {
                 ListResult = FacialMapRoomDatabase.getDatabaseMapa(mContext).pacienteDao().GetEmailList();
                 break;
             }
         }
