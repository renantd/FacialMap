...
public class DesenharPontoPreenMapa  extends View {
    ...
    public DesenharPontoPreenMapa(Context context,Activity activity) {
        super(context);
        this.mContext = context;
        this.activity = activity;
        initView();
    }
    public DesenharPontoPreenMapa(Context context) {
        super(context);
        this.mContext = context;
        initView();
    }
    public DesenharPontoPreenMapa(Context context, AttributeSet attributeSet){
        super(context,attributeSet);
        this.mContext = context;
        initView();
    }
    public DesenharPontoPreenMapa(Context context, AttributeSet attrs, int defStyle) {
        this(context, attrs);
        this.mContext = context;
        initView();
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (String x : ListCoord) {
            ...
            if(!x.equalsIgnoreCase("Selecione um ponto")){
              canvas.drawCircle(RefactorCoordX(x),RefactorCoordY(x), 15, paint);
              canvas.drawText(GetPointNumber(x),RefactorCoordX(x)+10, RefactorCoordY(x)-10,paint);
